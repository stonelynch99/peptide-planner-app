 ...
 const [discardEdits,setDiscardEdits]=useState(false);
  ...
  return (
    <SafeAreaProvider><SafeAreaView style={styles.safe}>
      ...
      <View style={styles.saveSyncRow}><Text testID="save-status" style={styles.smallBadge}>{saved.saving?'Saving on device…':saved.error?saved.error:'Saved on this device'}</Text>{betaAccount.state.status==='eligible'&&<Pressable accessibilityRole="button" accessibilityLabel={cloudSync.state.label+'. '+cloudSync.state.detail} onPress={cloudSync.activate} style={[styles.syncButton,cloudSync.state.kind==='needsAttention'&&styles.syncButtonAttention]}><Text style={styles.syncButtonText}>↻ {cloudSync.state.label}</Text></Pressable>}{!!saved.error&&!saved.loadFailed&&<AppButton label="Retry save" onPress={()=>saved.retry().catch(()=>{})} secondary/>}{!!reminderError&&<Text style={styles.smallBadge}>{reminderError}</Text>}</View>
